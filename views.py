...
def problem_one(request):
  smart_students = Student.objects.filter(gpa__gt=3.0).order_by("-gpa")
...
def problem_two(request):
    vet_instructors = Instructor.objects.filter(hire_date__lt="2010-01-01")
    for instructor in vet_instructors:
...
def problem_three(request):
  ...
  instructor_pk2 = Instructor.objects.filter(pk=2)
  for instructor in instructor_pk2:
    print(f"Instructor Name: {instructor.first_name} {instructor.last_name}")
  course_by_pk2 = Course.objects.all().filter(pk=2)
  for course in course_by_pk2:
    print(f"Courses:\n\t-{course.name}")
...
def problem_four(request):
  ...
  print(f"Student Count: {total_students}")
  total_courses = Course.objects.all().count()
  print(f"Courses Count: {total_courses}")
  total_instructors = Instructor.objects.all().count()
  print(f"Courses Count: {total_instructors}")
...
def problem_five(request):
  student = Student.objects.create(first_name="Julia", last_name="Narine", year= 2024, gpa=3.6)
...
def problem_seven(request):
    ...
    # Make sure to set this equal to the primary key of the row you just created!
    try:
        student = Student.objects.get(pk=11).delete()
